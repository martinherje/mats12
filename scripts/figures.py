...
# 2. the cross
fig, ax = plt.subplots(figsize=(9, 4.2)); x = np.arange(len(DESIGNS)); w = 0.36
...
ax.axhline(0.5, color="gray", lw=0.6, ls="--"); ax.set_xticks(x); ax.set_xticklabels([l for _, l in DESIGNS], fontsize=9); ax.set_ylim(0, 1.05); ax.set_ylabel("cross-stratum test AUROC (held-out topics)")
ax.set_title(f"The 2×2 cross: train inside one stratum, test on the other, topics never seen{sup}"); ax.legend(fontsize=7, loc="upper right", ncol=2)
...
try:
    ...
    for j, (cond, name, col) in enumerate(conds):
        ...
        axes[2].bar(j + 2.2, au, width=0.6, color="gray", alpha=0.6 + 0.2 * j, label="model's own Yes/No logit, same rows" if j == 0 else None)
except Exception as e: axes[2].text(0.5, 0.9, f"(no just-ask file: {e})", fontsize=7, transform=axes[2].transAxes)
axes[2].axhline(0.5, color="gray", ls="--", lw=0.8); axes[2].set_ylim(0, 1.05); axes[2].set_xticks([0.5, 2.7]); axes[2].set_xticklabels(["probe", "just ask"]); axes[2].set_title("Fair baseline on the headline's test rows\n(AUROC, legal vs illegal, harmless stratum)", fontsize=9); axes[2].legend(fontsize=7)

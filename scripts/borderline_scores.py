...
IN PLAIN LANGUAGE
The labels are 0/1, but the probe's output is a continuous score (positive = called legal, negative = called
illegal; the further from zero, the more confident). If the model represents nuance, the sentences Martin
...
print("\nThe ten design sentences closest to the line (score ≈ 0 = the probe cannot decide):")
...
ax.axvline(0, color="k", lw=0.8); ax.set_xlabel(f"probe score (− = called {'illegal' if a.target == 'legal' else 'harmless'}, + = called {a.target}); held-out topics"); ax.set_ylabel("sentences"); ax.legend(); ax.set_title(f"Do borderline sentences sit closer to the line? {a.run}, layer {L}", fontsize=9)

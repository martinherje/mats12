"""borderline_scores.py — are the borderline sentences the ones the probe is least sure about?

IN PLAIN LANGUAGE
The labels are 0/1, but the probe's output is a continuous score (positive = called illegal, negative = called
legal; the further from zero, the more confident). If the probe's score tracks how clear a case is, the sentences
flagged as borderline should get scores closer to zero than the clear ones. This script refits the
...
a.target = ev["args"]["target"]   # what the probe predicts, read from the evaluation so the refit matches it
...
print("\nThe ten design sentences closest to the line (scores near zero; + means called illegal):")

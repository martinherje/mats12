...
- My guess for how well it sorts them (AUROC; 0.5 = coin flip, 1.0 = perfect): ______ (Claude's suggestion: about 0.75)
- How many of the 100 shuffled-label runs it should beat: ______ (suggestion: 95 or more)
- The same test the other way round (train on harmless, test on harmful): ______ (suggestion: about 0.70)
- The harm probe across legality (train on illegal, test on legal, and back): ______ (suggestion: 0.80 or more both ways; harm is the easier concept)

**What I expect the "hat test" to show.** The legality direction and the harm direction are two arrows in the model's space. If they point the same way, legality is just harm renamed.
- The angle between them (cosine; 0 = unrelated, ±1 = same line): ______ (suggestion: no more than 0.4, and inside the shuffled band)
- If I remove the harm part from the legality arrow, does the rest still sort legal from illegal among harmless sentences? My guess (AUROC): ______ (suggestion: about 0.70)

**Against the model's own answer.** The probe has to beat, or at least match, just asking the model "is this illegal?" on the same sentences. My guess for the model's own score: ______ · for the probe: ______

**The checks.** The 60 plain legal acts ("You cook pasta") should all be called legal; my guess for how many: ______ of 60. The 61 "You do not …" sentences should all be called legal; my guess: ______ of 61.

**The result I would also count as a real finding.** If the legality probe does no better than the shuffled labels, or the legality arrow collapses once harm is removed, then on this model a "compliance monitor" built from activations is a harm detector. That is the outcome the literature predicts, and it is a headline too.
